...
export default function DashboardPage() {
  ...
  const role = data?.session?.user.role ?? "tenant";

  return (
    <section className="grid gap-6 md:grid-cols-3">
      <Card><Card.Content><h2 className="text-xl font-semibold">Welcome back</h2><p className="text-sm text-slate-600">{data?.session?.user.name}</p></Card.Content></Card>
